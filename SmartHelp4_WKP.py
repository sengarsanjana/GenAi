...
# Function to summarize transcript text into key points
def summarize_transcript(transcript_text):
    sentences = re.split(r'(?<=[.?!])\s+', transcript_text.strip())
    key_points = sentences[:3]  # Taking top 3 as summary points
    return key_points
    ...
    if results:
        ...
        for i, (video, score) in enumerate(results):
            with st.expander(f"Video {i+1}: {video['title']}", expanded=(i == 0)):
                ...
                # --- Show transcript summary as key points ---
                transcript_text = video.get("transcript", {}).get("full_text", "")
                if transcript_text:
                    key_points = summarize_transcript(transcript_text)
                    st.markdown("**📝 Key Points from Transcript:**")
